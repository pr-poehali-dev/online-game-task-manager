...
### Этап 5 — Бюджеты и админ-контроль ✅ Выполнено

- [x] Кабинет → Команда: в карточке сотрудника (`UserList.tsx`) — поле "Лимит AI: X ₽/мес",
      видно только если у сотрудника уже включено право `ai_access` (иначе лимит не имеет
      смысла). Inline-редактирование по паттерну `editSpecId`/`saveSpec` →
      `editAiLimitId`/`editAiLimitValue`/`saveAiLimit` (`useTeamManagement.ts`).
- [x] `backend/admin/index.py`: `action=set_ai_limit` — принимает `user_id`+`limit_rub`, пишет в
      `ai_usage` СТРОКУ ТЕКУЩЕГО МЕСЯЦА (`INSERT ... ON CONFLICT (user_id, month) DO UPDATE`, т.к.
      лимит в БД хранится помесячно вместе с фактическим расходом, см. Этап 1). Доступно
      `team_manage`/`admin` (общий guard `has_team_access` на весь backend/admin/index.py).
      GET-список пользователей дополнительно возвращает `ai_limit_rub` каждому (300₽ по
      умолчанию, если лимит ещё не задавался).
- [x] `action=ai_usage_summary` в `backend/admin/index.py` — сводка трат ВСЕХ сотрудников за
      текущий месяц одним запросом (`LEFT JOIN users ... ai_usage`, сортировка по убыванию
      расхода). Раздел Кабинет → Статистика → блок "Траты на AI за этот месяц"
      (`CabinetStats.tsx`, компонент `AiUsageSection`) — список сотрудников с прогресс-барами
      расхода (цвет меняется на жёлтый/красный при приближении к лимиту), плюс общий остаток
      баланса аккаунта AI Tunnel рядом с заголовком (`action=balance` из `backend/ai/index.py`).
      Виден только при `hasTeamAccess`.
- [x] Счётчик "Потрачено X из Y ₽" и блокировка отправки при исчерпании лимита на стороне
      сотрудника — уже было сделано на Этапе 3 (`AiComposer.tsx`/`AiGenerateComposer.tsx`,
      `usage`/`limitExceeded` в `Ai.tsx`), здесь только донастроено под новую возможность
      администратора менять лимит на лету.
- [x] Проверено вживую через API: `set_ai_limit` меняет `limit_rub` в `ai_usage`,
      `ai_usage_summary` возвращает сводку по всем сотрудникам, `action=balance` отдаёт реальный
      остаток аккаунта AI Tunnel (3030₽ на момент проверки).

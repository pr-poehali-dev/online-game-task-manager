...
### Этап 6 — Полировка ✅ Выполнено

- [x] Обработка ошибок AI Tunnel — `AITUNNEL_STATUS_MESSAGES` в `Ai.tsx` (по паттерну
      `ERROR_MESSAGES` в `LogsTypes.ts`/`Logs.tsx`): backend уже пробрасывает реальный HTTP-статус
      AI Tunnel как statusCode ответа (см. `_aitunnel_request`/`_aitunnel_error` в
      `backend/ai/index.py`), фронт различает 400 (модель не приняла запрос) / 401 (недействителен
      ключ — сообщить админу) / 402 (закончился баланс аккаунта — сообщить админу) / 429
      (перегружена, повторить позже), для остальных статусов — текст `message` от AI Tunnel как
      есть. Также добавлены тексты для `unauthorized`/`bad_request`/`not_found` — раньше падали в
      общий "Не удалось выполнить запрос".
- [x] Пустые состояния:
  - `AiChatList.tsx` — было и осталось "Пока нет ни одного диалога" (плюс отдельное состояние
    "Ничего не найдено" для нового поиска, см. ниже).
  - `AiMessageList.tsx` — раньше один общий текст на всё, теперь `EMPTY_STATE` со своей
    иконкой/подсказкой под каждый режим (`chat`/`code`/`image`/`video`) — объясняет, что именно
    делать в конкретной вкладке, плюс показывает `sendError` прямо в пустом состоянии (раньше
    ошибка на пустом чате была не видна, т.к. блок ошибки рендерился только среди сообщений).
  - `Ai.tsx` — состояние `forbidden` (нет права `ai_access`) уже было готово с Этапа 3, не
    изменялось.
- [x] Мобильная адаптация — `AiChatList` на экранах `< lg` скрыт (`hidden lg:flex`), вместо него
      кнопка-гамбургер (иконка `PanelLeft`) в шапке раздела открывает тот же `AiChatList` внутри
      `Sheet` (паттерн 1:1 с мобильным меню в `IndexTopbar.tsx`/`Cabinet.tsx`) — новый проп
      `bare` у `AiChatList` убирает фиксированную ширину/бордер колонки, когда она внутри Sheet.
      Рядом с кнопкой на мобильном виден заголовок текущего диалога. Кнопки закрепить/
      переименовать/удалить у каждого чата раньше показывались только по hover
      (`group-hover:flex`) — на тач-экранах их вообще нельзя было нажать; теперь на `< lg` они
      видны всегда, на `lg+` — по-прежнему по наведению.
- [x] Закрепление диалогов (`ai_chats.pinned`) — было реализовано ещё на Этапе 3
      (`onTogglePinned`/кнопка-булавка в `AiChatList.tsx`), здесь не менялось. Добавлен **поиск по
      истории чатов** — строка поиска над списком в `AiChatList.tsx` (появляется, когда диалогов
      больше 4, чтобы не занимать место при малом количестве чатов), фильтрует по заголовку.
- [x] Проверено: `npx tsc --noEmit` и `eslint` без ошибок по всем изменённым файлам, приложение
      собирается и рендерится (проверено скриншотом страницы входа).

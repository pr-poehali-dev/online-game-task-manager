...
def handler(event: dict, context) -> dict:
    ...
    if method == 'GET':
        ...
        cur.execute(
            ...
            f"u.ai_file_limit, u.ai_size_limit_mb, u.ai_project_limit, "
            f"(SELECT COUNT(*) FROM {schema}.ai_projects p WHERE p.user_id = u.id AND p.archived = false) AS ai_projects_used, "
        ...
        for r in rows:
            ...
            users.append({
                ...
                'ai_project_limit': int(r[23]) if r[23] is not None else 10,
                'ai_projects_used': int(r[24] or 0),
                'ai_files_used': int(r[25] or 0),
                'ai_size_used_mb': round(int(r[26] or 0) / (1024 * 1024), 1),
            })
        cur.close(); conn.close()
    ...
    if action == 'set_role':
        ...
        if size_limit_mb < 0:
            cur.close(); conn.close()
            return {'statusCode': 400, 'headers': _cors_headers(), 'body': json.dumps({'error': 'bad_limit'})}
        cur.execute(f"UPDATE {schema}.users SET ai_size_limit_mb = %s, updated_at = NOW() WHERE id = %s", (size_limit_mb, user_id))
        _log_activity(cur, schema, admin_id, 'user_set_ai_size_limit', 'user', user_id, _target_name, f'{size_limit_mb} МБ')
    elif action == 'set_ai_project_limit':
        # Сколько ПРОЕКТОВ сотрудник может держать в разделе "AI" (users.ai_project_limit, см.
        # db_migrations V0084). Архивные проекты в лимит не считаются. 0 — создание запрещено.
        try:
            project_limit = int(body.get('project_limit'))
        except (TypeError, ValueError):
            cur.close(); conn.close()
            return {'statusCode': 400, 'headers': _cors_headers(), 'body': json.dumps({'error': 'bad_limit'})}
        if project_limit < 0:
            cur.close(); conn.close()
            return {'statusCode': 400, 'headers': _cors_headers(), 'body': json.dumps({'error': 'bad_limit'})}
        cur.execute(f"UPDATE {schema}.users SET ai_project_limit = %s, updated_at = NOW() WHERE id = %s", (project_limit, user_id))
        _log_activity(cur, schema, admin_id, 'user_set_ai_project_limit', 'user', user_id, _target_name, f'{project_limit} проектов')

...
- `ddf_parser.py` (DDF-парсер + disassemble/assemble + потоковые iter_records/get_record_count/
  ...
  delete_record/append_records/get_tail_bytes + MTX/MAT + ENBBY + cp1251-mojibake автофикс
  ASCF-полей) — готово. Массово проверен на HF (28/28) и на C4 (42/42 простых файла +
...
- Backend actions `ddf_search` (с offset/hasMore для пагинации списка результатов)/`ddf_get`/
  `ddf_save`/`ddf_new`/`ddf_create` (обычные rows + rawLines для RAW_ONLY, с проверкой
  дубликатов id по `_ID_FIELDS`)/`ddf_delete`/`ddf_get_raw`/`ddf_save_raw` (тоже с проверкой
  дубликатов id) в `index.py` — РЕАЛИЗОВАНЫ для ОБОИХ клиентов (реестр выбирается по серверу
  через `_ddf_match()`), задеплоены, протестированы на живых серверах hfx3old И c4x1. Готово.

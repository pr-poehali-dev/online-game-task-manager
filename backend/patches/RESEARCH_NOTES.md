...
- `ddf_parser.py` (DDF-парсер + disassemble/assemble + потоковые iter_records/get_record_count/
  ...
  get_tail_bytes + MTX/MAT + ENBBY поддержка) — готово. Массово проверен на HF (28/28) и на C4
  (42/42 простых файла + armorgrp/etcitemgrp/recipe-c с MTX/MAT + weapongrp с ENBBY + npcgrp,
  оба метода: disassemble+assemble и потоковый transform_single_row).
...
- `ddf_registry_c4.py` (реестр DDF-схем для C4/Chronicle 4) — 33 схемы (25 обычных + armorgrp/
  ...
  включая 5 с fixed_record_count (eula, chargrp, hairgrp, helmetgrp, logongrp) и 8
  RAW_ONLY_SCHEMAS. Готово, проверено на реальных данных C4x1.

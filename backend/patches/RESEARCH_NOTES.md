...
- `ddf_parser.py` (DDF-парсер + disassemble/assemble + потоковые iter_records/get_record_count/
  ...
  get_tail_bytes) — готово, поддерживает файлы как с reccnt-префиксом, так и с фиксированным
  числом записей (fixed_record_count). Массово проверен на HF (28/28) и на C4 (42/42 файла,
...
- `ddf_registry_c4.py` (реестр DDF-схем для C4/Chronicle 4) — 25 схем, включая 2 с
  fixed_record_count (eula, chargrp). Готово, проверено на реальных данных C4x1.
- Backend actions `ddf_search`/`ddf_get`/`ddf_save`/`ddf_new`/`ddf_create`/`ddf_delete` в
  `index.py` — РЕАЛИЗОВАНЫ для ОБОИХ клиентов (реестр выбирается по серверу через
  `_ddf_match()`), задеплоены, протестированы на живых серверах hfx3old И c4x1. Готово.
- Frontend UI (`PatchesDdfEditor.tsx`) — работает для обоих клиентов без изменений (просто
  передаёт `server` в каждый API-вызов, backend сам выбирает нужный реестр схем). Готово.
...
## ЭТАП 15 — В ПРОЦЕССЕ: перенос поддержки "сложных" файлов (MTX/MAT/ENBBY) на HF-клиент

### Задача
Пользователь: "мы адаптировали раздел патчей под C4 сервер, но HF остался не тронут" — перенести
все наработки (MTX/MAT-парсинг, ENBBY, RAW-режим и т.п.) с C4 на обычный HF-реестр
(`ddf_registry.py`, используется для серверов hfx3old/hfnew, НЕ привязан к конкретному серверу в
отличие от `ddf_registry_c4.py`).

### Матчасть: что уже работало на HF без изменений
`ddf_registry.py` (основной реестр) уже поддерживал 74 "простых" файла (itemname/skillname/
npcname/questname и т.п., см. комментарий в начале файла) — эти файлы для HF УЖЕ РАБОТАЮТ без
каких-либо правок, ничего переносить не нужно. Не хватало ТОЛЬКО 7 файлов, изначально осознанно
исключённых из реестра как использующие неподдержанные типы: `armorgrp`, `etcitemgrp`, `recipe-c`,
`npcgrp`, `weapongrp`, `vehiclepartsgrp`, `mantleexception`.

### Источники данных (получены от пользователя в этой сессии)
1. `DAT_defs.zip` — официальные DDF-описания (H5pt) для ВСЕХ 81 файла HF-клиента.
2. `L2_File_Editor_HF.zip` — официальные TXT-экспорты l2disasm для сверки: `armorgrp.txt`,
   `etcitem.txt`, `npcgrp.txt`, `recipe-c.txt`, `vehiclepartsgrp.txt`, `weapongrp.txt` (НЕТ
   эталона для `mantleexception` — раскрыт экспериментально другим методом, см. ниже).
3. Реальные боевые .dat файлы уже загружены на сервер `hfx3old` (109 файлов, включая все 7
   "сложных" + неожиданный доп. файл `npcgrp_def.dat`, отсутствующий в DDF_defs.zip).

### КЛЮЧЕВОЕ ОТКРЫТИЕ: HF-версии сложных файлов используют ДВА НОВЫХ типа полей
Сравнение HF DDF с уже готовыми C4-схемами тех же файлов показало: `etcitemgrp`/`weapongrp`/
`npcgrp` на HF используют ТЕ ЖЕ типы, что уже поддержаны (MTX/MAT/ENBBY простой синтаксис) — их
можно перенести напрямую. НО `armorgrp` (HF) использует НОВЫЙ тип `MTX3` (вместо `MTX` у C4),
`recipe-c` (HF) использует НОВЫЙ тип `MAT2` (вместо `MAT` у C4), а `mantleexception` использует
РАСШИРЕННЫЙ синтаксис `ENBBY` с двоеточием (`ENBBY = [(field:threshold,N)]`) — ни один из этих
трёх форматов не был раскрыт раньше (docstring ddf_parser.py прямо указывал "MTX2/MTX3/MAT2 не
поддержаны — ни одна из известных схем C4/HF их не использует, встретить не удалось" — оказалось,
что HF armorgrp/recipe-c именно их и используют).

### Методология раскрытия (тот же подход, что и на предыдущих этапах — экспериментально +
побайтовая сверка с официальным TXT-экспортом, НЕ гадание по сырым байтам)
Скачан реальный зашифрованный `armorgrp.dat`/`recipe-c.dat`/`mantleexception.dat` с сервера
`hfx3old` через прямую CDN-ссылку (`patch_files.file_key`), расшифрован локально через
`l2encdec.decode` (protocol 413, тот же алгоритм, что и для C4 — RSA-ключ общий для клиента).
Разобран вручную по байтам с опорой на порядок и названия колонок из официального TXT-заголовка
(`{name}_cntm`, `{name}_mU[i]`, `{name}_mB[i][1]`, `{name}_mB[i][2]`, `{name}_cntt`, `{name}_t[i]`,
`{name}_tE` для MTX3 в armorgrp.txt; `materials_cnt`, `materials_extra`, `materials_m[i]_id`,
`materials_m[i]_cnt` для MAT2 в recipe-c.txt).

### MTX3 — раскрыт и подтверждён (armorgrp.dat, 3751 записей на hfx3old)
Формат: `UINT count1, count1 x (UNICODE, UCHAR, UCHAR)` (mesh-подтаблица — КАЖДЫЙ элемент теперь
ТРОЙКА данных вместо простой строки, как в обычном MTX), `UINT count2, count2 x UNICODE`
(tex-подтаблица — как в обычном MTX), `UNICODE` (ОДНО дополнительное поле в самом конце записи
целого MTX3-блока, не связанное со счётчиком). Подтверждено побайтовым разбором первой записи
armorgrp (id=21): `m_HumnFigh_add_cntm=1`, `mU[0]=''`, `mB[0][1]=0`, `mB[0][2]=255` — байты
`01000000 00000000 00 ff 01000000 00000000000000 01000000...` разобраны точно, следующее поле
(`att_eff` = "LineageEffect.p_u002_a") тоже совпало с TXT. Массовая проверка: РУЧНОЙ парсер (34
MTX/MTX3-поля на запись — 17 расо-полов х 2) на первых 30 записях — id/pdef (последнее поле
каждой записи) совпали со всеми 30 записями TXT без единого расхождения. После интеграции в
`ddf_parser.py` (см. ниже) — ПОЛНЫЙ `disassemble`+`assemble` round-trip на ВСЕХ 3751 записях —
**byte-perfect** (реконструированный файл `==` оригинал побайтово).

### MAT2 — раскрыт и подтверждён (recipe-c.dat, 1001 запись на hfx3old)
Формат: `UINT count, UINT extra, count x (UINT id, UINT amount)` — та же структура пар id/amount,
что и в обычном MAT, но с ОДНИМ дополнительным `UINT`-полем ("extra") СРАЗУ после счётчика (перед
списком пар). Во ВСЕХ проверенных записях `extra=0` — назначение поля не раскрыто, но формат
хранения (позиция, размер) подтверждён однозначно. Массовая проверка: ВСЕ 1001 записей recipe-c
разобраны и сверены с TXT (materials_cnt/materials_extra/materials_m[i]_id/materials_m[i]_cnt) —
0 расхождений. После интеграции — полный `disassemble`+`assemble` round-trip — **byte-perfect**.

### Расширенный ENBBY с "порогом" — раскрыт и подтверждён (mantleexception.dat, 92 записи,
БЕЗ официального TXT-эталона — раскрыт другим методом, см. ниже)
Синтаксис в DDF: `ENBBY = [(cond_field:threshold,N)];`, ЧАСТО НЕСКОЛЬКО таких строк подряд для
ОДНОГО поля с разными N (например `rcid_5` имеет 3 строки с N=5,6,8). Гипотеза выдвинута по
структуре имён полей схемы (`rcid_1..rcid_13`/`rctex_1..rctex_13` — 13 повторяющихся пар,
управляемых общим счётчиком `txt_cnt`) и проверена БЕЗ TXT-эталона следующим методом: разобран
файл вручную с правилом "поле активно, если `txt_cnt >= K`" (K — номер пары), после чего сверено
смещение КОНЦА последней записи с РЕАЛЬНЫМ концом файла — оно СОВПАЛО ТОЧНО С ГРАНИЦЕЙ служебного
хвостового маркера `"SafePackage"` (13 байт ASCF-строки, стандартный маркер конца .dat-файлов) —
что математически возможно только при абсолютно точном разборе КАЖДОГО байта всех 92 записей до
этой точки (любая ошибка в структуре сдвинула бы все смещения, и хвост считался бы как мусор).
Дополнительно автоматически извлечены все `(cond_field, threshold, N)` тройки из оригинального
DDF-текста регулярным выражением и подтверждено: минимальное N среди всех ENBBY-строк одного поля
ВСЕГДА равно порядковому номеру этой пары (K) — что и есть найденное правило `cond_field >= K`.
Значение threshold (число после двоеточия) экспериментально НЕ влияет на подтверждённое
byte-perfect поведение — сохраняется, но игнорируется при вычислении активности поля.

### Изменения в `ddf_parser.py`
- Docstring модуля обновлён: MTX3/MAT2 теперь ЧИСЛЯТСЯ как поддержанные (было "не поддержаны").
- `parse_ddf()`: убрана ошибка `unsupported_type_MTX3`/`unsupported_type_MAT2` (оставлена только
  для `MTX2`, который так и не встретился ни в одной известной схеме).
- Новый `_ENBBY_THRESHOLD_RE` — распознаёт `ENBBY = [(field:threshold,N)];` ОТДЕЛЬНО от старого
  `_ENBBY_RE` (`ENBBY = [(field,N)];`, без двоеточия). Каждое поле теперь имеет доп. флаг
  `enbby_gte` (bool) — False для старого простого синтаксиса (сравнение `==`), True для нового
  расширенного (сравнение `>=`). При НЕСКОЛЬКИХ ENBBY-строках для одного поля с `enbby_gte=True`
  — берётся МИНИМАЛЬНОЕ N (см. правило выше), реализовано аккумулированием в цикле парсинга.
- `_enbby_active(field, row)`: ветвится по `enbby_gte` — `==` для старого синтаксиса, `>=` для
  нового.
- Новые функции `_read_mtx3`/`_write_mtx3` (мест использования: `_read_field`/`_write_field`/
  `default_row`, по аналогии с уже существующими `_read_mtx`/`_write_mtx`) и `_read_mat2`/
  `_write_mat2` (аналогично `_read_mat`/`_write_mat`). Row-представление: MTX3 —
  `{'mesh': [{'u','b1','b2'}, ...], 'tex': [...], 'tail': str}`; MAT2 —
  `{'extra': int, 'items': [{'id','amount'}, ...]}`.

### Изменения в `ddf_raw.py` (RAW-режим редактирования одной записи текстом)
`_row_to_pairs()` и `raw_line_to_row()` дополнены веткой для MTX3 (колонки `{name}_cntm`,
`{name}_mU[i]`, `{name}_mB[i][1]`, `{name}_mB[i][2]`, `{name}_cntt`, `{name}_t[i]`, `{name}_tE` —
имена СОВПАДАЮТ с реальными колонками HF TSV-экспорта l2disasm) и MAT2 (`{name}_cnt`,
`{name}_extra`, `{name}_id[i]`, `{name}_amount[i]`).

### Регрессионное тестирование (после всех изменений в общем `ddf_parser.py`, используемом ОБОИМИ
реестрами — C4 и HF)
Повторно прогнан полный `disassemble`+`assemble` round-trip на РЕАЛЬНЫХ C4-файлах:
1. `weapongrp.dat` (C4, простой ENBBY-синтаксис, 1134 записей) — **byte-perfect**, не сломался.
2. `armorgrp.dat` (C4, обычный MTX без ENBBY, 1352 записей, с учётом 2-байтного
   ARMORGRP_TRAILING_QUIRK_BYTES) — **byte-perfect**, не сломался.
Изменения в `ddf_parser.py` (новые типы MTX3/MAT2, новый _ENBBY_THRESHOLD_RE) полностью обратно
совместимы — существующая функциональность C4 не затронута.

### Дополнительно раскрытые HF-файлы (уже известные типы, простой перенос C4→HF)
Проверены отдельно на реальных данных `hfx3old`, все **byte-perfect** на ПОЛНОМ файле:
- `etcitemgrp.dat` (обычный MTX, 11615 записей).
- `weapongrp.dat` (обычный MTX + простой ENBBY, 4060 записей) — HF-версия структурно шире C4
  (дополнительные поля `newdata[8]`, `UNK_2_tab[]`, `wpn_unkval[]`, `icons[4]` и т.п.).
- `npcgrp.dat` (динамические массивы + простой ENBBY на `rb_effect`/`rb_effect_fl`, 10471
  записей) — та же опечатка дублирующегося имени поля `tex1`→`tex1`, что и в C4-версии,
  исправлена на `tex2` тем же способом.
- `npcgrp_def.dat` (ЗАГАДКА РАЗГАДАНА: не упомянут в DAT_defs.zip, но структурно идентичен
  `npcgrp.dat` — та же схема даёт byte-perfect round-trip, 10466 записей; похоже на
  резервную/дефолтную версию той же таблицы данных о мобах на этом конкретном сервере).
- `vehiclepartsgrp.dat` (обычный MTX + статические/динамические массивы, всего 4 записи).

### Raw round-trip (row -> raw line -> row, через обновлённый ddf_raw.py)
Проверено на ВСЕХ записях armorgrp.dat (3751, MTX3) и recipe-c.dat (1001, MAT2) — 0 расхождений.

### Регистрация схем в `ddf_registry.py` (основной HF-реестр)
Все 8 схем (7 из DAT_defs.zip + `npcgrp_def` как алиас на `npcgrp`) добавлены в `_DDF_TEXTS`
дословно из официального пака (с той же опечаткой-фиксом `tex1`→`tex2` в npcgrp, что и в C4-
версии). `_EDITABLE_TEXT_FIELDS`: `armorgrp`/`etcitemgrp`/`weapongrp`/`npcgrp`/`npcgrp_def` — []
(нет текстовых полей), `recipe` — `['name']`, `vehiclepartsgrp` — `['name']` (единственное
осмысленное текстовое поле помимо путей к моделям в MTX), `mantleexception` — все 13
`rctex_N`-полей (короткие ASCF-идентификаторы текстур, но схема всё равно отнесена к RAW_ONLY —
см. ниже, из-за сложной условной структуры записи). `_ID_FIELDS`: `armorgrp`→`['id']`,
`etcitemgrp`→`['id']`, `recipe`→`['id_recipe']`, `weapongrp`→`['id']` (перенесены 1-в-1 с C4-
реестра); `npcgrp`/`npcgrp_def`/`vehiclepartsgrp`/`mantleexception` НЕ включены (нет проверенного
понятия "id" — npcgrp вообще не содержит числового id-поля, только tag/class/mesh).

Новые константы `FIXED_RECORD_COUNTS = {}` (пусто — ни у одной новой HF-схемы нет RECCNT=N в
DDF) и `RAW_ONLY_SCHEMAS = {'armorgrp', 'etcitemgrp', 'recipe', 'weapongrp', 'npcgrp',
'npcgrp_def', 'vehiclepartsgrp', 'mantleexception'}` (все 8 новых схем — включая
`mantleexception`, у которой ЕСТЬ editable-поля, но сложная условная ENBBY-структура записи
удобнее для правки целиком одной строкой, как и остальные raw-only схемы).

`match_ddf()` в `ddf_registry.py` переделан с 3-кортежа на 6-кортеж (`schema_key, fields,
editable, has_reccnt_prefix, fixed_record_count, is_raw_only`) — `_ddf_match()` в `index.py`
УЖЕ поддерживал этот формат заранее (написан универсально ещё на этапе поддержки C4), изменений
в `index.py` не потребовалось вообще, только обновлён docstring.

### Проверка ARMORGRP_TRAILING_QUIRK_BYTES для HF — квирк НЕ нужен
В отличие от C4 `armorgrp.dat` (у одного конкретного пользователя — 2 лишних байта в конце файла
как защита от воровства, см. ЭТАП 6), HF `armorgrp.dat` на сервере `hfx3old` расшифровался
СРАЗУ, без какого-либо страйпинга хвостовых байт (`l2encdec.decode` на полном файле сработал с
первой попытки, что и подтвердило byte-perfect round-trip на всех 3751 записях) — `_ddf_quirk_bytes`
для HF-серверов не требуется, специальной обработки в `index.py` добавлять не нужно.

### Регрессионное тестирование реестра (после регистрации всех 8 новых схем)
Локально через `python3 -c "import ddf_registry; ..."`:
- Все 8 новых схем (`armorgrp`/`etcitemgrp`/`recipe-c`/`weapongrp`/`npcgrp`/`npcgrp_def`/
  `vehiclepartsgrp`/`mantleexception`) корректно матчатся, `is_raw_only=True`, `idFields`
  соответствуют ожиданиям.
- 6 случайных СТАРЫХ схем (`itemname`/`skillname`/`npcname`/`questname`/`eula`/`actionname`) —
  по-прежнему матчатся, `is_raw_only=False`, число полей не изменилось.
- `list_supported_keys()` — было 74, стало 82 (74 + 8 новых) — согласуется с ожиданием.

### Деплой и end-to-end тесты на живом сервере hfx3old — ВСЕ ПРОЙДЕНЫ
После `sync_backend` (13/13 → 15/15 тестов tests.json, без изменений в самих тестах — просто
подтверждение, что деплой не сломал существующий контракт):
1. `ddf_search` (пустой запрос) на всех 8 схемах — `totalRows` СОВПАДАЕТ с локально подтверждённым
   byte-perfect количеством записей (armorgrp=3751, etcitemgrp=11615, recipe-c=1001,
   weapongrp=4060, npcgrp=10471, npcgrp_def=10466, vehiclepartsgrp=4, mantleexception=92),
   `isRawOnly=True` у всех.
2. `ddf_get_raw` (index=0) на всех 8 схемах — корректное число колонок и содержимое строки,
   включая развёрнутые MTX3-поля (armorgrp: 247 колонок) и MAT2-поля (recipe-c: 14 колонок).
3. Полный цикл `ddf_save_raw` (изменить -> проверить -> вернуть обратно -> проверить) — ПРОЙДЕН
   на armorgrp (pdef: 36→99999→36), recipe-c (MAT2 materials_amount[0]), mantleexception (ENBBY
   rcid_1), npcgrp (FLOAT npc_speed — сравнение по значению, а не строке, т.к. FLOAT сериализуется
   как "1.0"), weapongrp (speed), etcitemgrp (weight), vehiclepartsgrp (unk1) — 7 схем, во всех
   случаях итоговое значение после отката ТОЧНО совпало с исходным.
4. Защита от дубликатов (`_ID_FIELDS`) подтверждена: попытка `ddf_create` с уже существующим
   `id=22` в armorgrp корректно отклонена (`duplicate_id_exists_id=22`).
5. Регрессия на уже работавшей простой схеме (`itemname-e.dat`, 19425 записей) — `ddf_search`
   отработал без изменений в поведении.

### Инцидент в процессе сессии: откат файлов ddf_parser.py/ddf_raw.py к исходному состоянию
В какой-то момент между вызовами инструментов `ddf_parser.py`/`ddf_raw.py` в ПРОЕКТЕ откатились к
версии ДО начала этой сессии (docstring снова говорил "MTX2/MTX3/MAT2 не поддержаны", файл стал
короче на ~180 строк) — вероятно, побочный эффект параллельной работы/переключения контекста
между запросами пользователя (аналогичный инцидент уже случался на ЭТАПЕ 5, см. выше). Обнаружено
случайной проверкой grep ПЕРЕД продолжением работы (хорошая практика — всегда проверять состояние
файла перед тем, как строить на нём следующий шаг). Исправлено копированием уже провалидированной
(byte-perfect на реальных данных) рабочей версии из временной директории `/tmp/hf_editor/` обратно
в проект через `cp`, с последующей повторной верификацией через Read-инструмент (чтобы убедиться,
что редактор действительно видит восстановленное содержимое) и повторным byte-perfect тестом на
armorgrp.dat — все 3751 записей подтверждены заново после восстановления.

### Статус
ЭТАП 15 ЗАВЕРШЁН. Все 7 "сложных" HF-файлов (+ npcgrp_def) полностью раскрыты, реализованы и
подтверждены byte-perfect как локально (полный disassemble+assemble round-trip на реальных
данных), так и через живой API на сервере hfx3old (поиск, просмотр, редактирование, защита от
дубликатов). Общее число поддерживаемых на HF схем выросло с 74 до 82 — теперь HF-клиент по
функционалу полностью паритетен с C4 (все типы полей MTX/MAT/MTX3/MAT2/ENBBY поддержаны в обоих
реестрах). Новые бинарные форматы (MTX3, MAT2, расширенный ENBBY с порогом) задокументированы в
docstring `ddf_parser.py` для будущих схем, которые могут их использовать.

### Не проверено / оставлено на будущее (не блокирует текущий этап, но стоит иметь в виду)
- `mantleexception` — назначение поля `threshold` (число после двоеточия в ENBBY) не раскрыто
  (не влияет на подтверждённое byte-perfect поведение, но потенциально несёт какой-то смысл в
  оригинальном формате/генераторе DDF).
- `MAT2` поле `extra` — назначение не раскрыто (во всех проверенных записях recipe-c.dat всегда
  0), формат хранения подтверждён, но семантика неизвестна.
- Файл `hfnew` (второй HF-сервер, отдельно от `hfx3old`) содержит только 1 загруженный файл на
  момент этой сессии — новые схемы актуальны и для него (тот же реестр `ddf_registry.py`, не
  привязан к конкретному серверу в отличие от C4), но живых данных для тестирования на `hfnew`
  пока нет.
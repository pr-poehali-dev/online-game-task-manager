...
- `ddf_registry_c4.py` (реестр DDF-схем для C4/Chronicle 4) — 31 схема (25 обычных + armorgrp/
  etcitemgrp/recipe с MTX/MAT + hairgrp/helmetgrp/logongrp), включая 5 с fixed_record_count
  (eula, chargrp, hairgrp, helmetgrp, logongrp) и 6 RAW_ONLY_SCHEMAS. Готово, проверено на
  реальных данных C4x1.
...
- Backend actions `ddf_search`/`ddf_get`/`ddf_save`/`ddf_new`/`ddf_create`/`ddf_delete`/
  ...
  Готово.
- Frontend UI (`PatchesDdfEditor.tsx` + `PatchesDdfRawPanel.tsx` (табличное представление
  "подпись колонки сверху / значение снизу", горизонтальная прокрутка) + обновлённый

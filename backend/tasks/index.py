...
SNIPPET_LEN = 100
# Набор эмодзи-реакций на комментарии ограничен фиксированным списком (как в мессенджерах) — так
# реакции остаются единообразными на всех карточках и не превращаются в произвольный текстовый ввод.
...
def _norm_servers(body, fallback=None):
    '''Приводит выбор серверов к списку строк без повторов. Задача может относиться сразу к
    нескольким серверам (одна правка выкатывается на все), поэтому фронт присылает servers —
    массив. Поле server оставлено для совместимости: если пришло только оно, считаем его списком
    из одного элемента. fallback — текущее значение задачи, когда в теле запроса серверов нет.'''
...
def handler(event: dict, context) -> dict:
    ...
    # og_meta — ЕДИНСТВЕННОЕ публичное (без токена) действие: отдаёт только номер и название
    # задачи по прямой ссылке /task/{id}, нужно для превью ссылки в Telegram/мессенджерах — их
    # боты не умеют войти в аккаунт, поэтому у них нет X-Auth-Token, и обычные защищённые
    # действия им недоступны. Намеренно НЕ отдаёт description и никакие другие поля задачи —
    # это внутренний задачник, наружу видно только id+title. Self-hosted-обёртка server.py
    # дёргает это действие, чтобы подставить og:title в HTML-ответ для краулера (см.
    # deploy/server.py, _is_bot_ua/_task_meta_html).
    if method == 'GET' and (qs.get('action') == 'og_meta'):
        task_id = qs.get('id')
        ...
        cur.execute(f"SELECT id, title FROM {schema}.tasks WHERE id = %s", (int(task_id),))
        row = cur.fetchone()
        cur.close(); conn.close()
        if not row:
            return {'statusCode': 404, 'headers': _cors_headers(), 'body': json.dumps({'error': 'not_found'})}
        return {'statusCode': 200, 'headers': _cors_headers(), 'body': json.dumps({'id': row[0], 'title': row[1]})}
    ...
    if action == 'create':
        ...
        cur.execute(
            ...
            (
                ...
                body.get('version'),
                # server хранит ПЕРВЫЙ сервер списка — на него опираются патчноуты и старые
    ...
    if action == 'archive':
        ...
        if task['column'] == 'restart' and outcome == 'done':
            for srv in (task.get('servers') or ([task['server']] if task.get('server') else [])):
                cur.execute(
                    f"INSERT INTO {schema}.patchnotes (server, task_id, task_title) VALUES (%s, %s, %s)",
                    (srv, int(task_id), task['title'])
                )
        cur.close(); conn.close()
        return {'statusCode': 200, 'headers': _cors_headers(), 'body': json.dumps({'task': task})}
    ...
    if action == 'comments':
        ...
        # Реакции — отдельным запросом по всем id разом (не по одному на комментарий), чтобы не
        # плодить N+1 при большом обсуждении. Группируем по (comment_id, emoji) с списком поставивших.
        reactions_by_comment: dict = {}
        if comment_ids:
            cur.execute(
                f"SELECT comment_id, emoji, user_id FROM {schema}.task_comment_reactions "
                f"WHERE comment_id = ANY(%s) ORDER BY created_at ASC",
                (comment_ids,)
            )
            for cid, emoji, uid in cur.fetchall():
                by_emoji = reactions_by_comment.setdefault(cid, {})
    ...
    # Эмодзи-реакция на комментарий, как в мессенджерах: повторный клик тем же пользователем на тот
    # же эмодзи убирает реакцию (toggle), а не плодит дубликаты — для этого в таблице уникальный
    # индекс (comment_id, user_id, emoji).
    if action == 'comment_reaction_toggle':
        cid = body.get('id')
        emoji = body.get('emoji')
        if not cid or emoji not in ALLOWED_REACTION_EMOJI:
            cur.close(); conn.close()
            return {'statusCode': 400, 'headers': _cors_headers(), 'body': json.dumps({'error': 'bad_request'})}
        cur.execute(f"SELECT task_id FROM {schema}.task_comments WHERE id = %s", (int(cid),))
        crow = cur.fetchone()
        if not crow:
            cur.close(); conn.close()
            return {'statusCode': 404, 'headers': _cors_headers(), 'body': json.dumps({'error': 'not_found'})}
        ...
        if me['role'] != 'admin' and not me['perms']['task_view_others']:
            ...
            if me['id'] not in own_ids and (not own_row or own_row[2] != me['id']):
                ...
                return _forbidden()
        cur.execute(
            f"DELETE FROM {schema}.task_comment_reactions WHERE comment_id = %s AND user_id = %s AND emoji = %s",
            (int(cid), me['id'], emoji)
        )
        added = cur.rowcount == 0
        if added:
            cur.execute(
                f"INSERT INTO {schema}.task_comment_reactions (comment_id, user_id, emoji) VALUES (%s, %s, %s) "
                f"ON CONFLICT (comment_id, user_id, emoji) DO NOTHING",
                (int(cid), me['id'], emoji)
            )
        cur.close(); conn.close()
        return {'statusCode': 200, 'headers': _cors_headers(), 'body': json.dumps({'ok': True, 'added': added})}

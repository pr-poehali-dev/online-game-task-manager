...
export function useDdfBulk(
  ...
  async function openBulk() {
    ...
    try {
      ...
      // raw-режиме (10 колонок). Вместо пустого шаблона берём за основу ПЕРВУЮ РЕАЛЬНУЮ запись
      // файла (тот же формат, что видно при просмотре/правке существующей записи текстом) —
      // гарантированно правильное число колонок для количества элементов массивов, которое
      // реально используется в файле.
      //
      // Id-поля в шаблоне НЕ очищаем (в отличие от "Дублировать") — пользователь массово
      // добавляет РАЗНЫЕ записи с разными id, ему нужно видеть реальный пример значения (и в
      // каком формате оно записывается), чтобы скопировать строку N раз и проставить свои id.
      // Проверка на дубликаты — целиком на backend (_ddf_check_duplicate_key), которая сравнивает
      // составной id-ключ (все id_field_names сразу, не только первое поле) — например для
      // skillname это пара (id, level): один и тот же skill id на разных уровнях — не дубликат,
      // а вот повтор ТОЙ ЖЕ пары (id, level) backend заблокирует. Если в файле вообще нет
      // записей — используем пустой шаблон от ddf_new как есть.
      try {
        const sample = await postJson({ action: 'ddf_get_raw', server, path, index: 0 });
        setBulkTemplateLine(sample.line ?? '');
        setBulkRawColumns(sample.columns || []);
      } catch {

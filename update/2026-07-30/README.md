...
5. **Ускорение переключения между разделами** — доска, спринты, архив,
   база знаний, патчноуты, патчи и т.д. теперь загружаются с сервера
   только один раз за визит, переключение между уже открытыми разделами
   происходит мгновенно, без крутящегося колеса
6. ⚠️ Новая **системная** зависимость на сервере — библиотека `libgmp-dev`
   (для быстрого шифрования больших `.dat`-файлов), см. раздел 7
...
### Важно: новая системная зависимость (см. раздел 7)
...
## 5. Ускорение переключения между разделами

### Что было не так
Каждый раздел («Доска», «Спринты», «Архив», «База знаний», «Патчноуты»,
«Патчи» и т.д.) монтировался заново при каждом переключении вкладки —
компонент создавался с нуля, заново запускался запрос данных к серверу,
из-за чего при каждом переходе (даже повторном, в уже открытый раздел)
на 1-2 секунды появлялся крутящийся индикатор загрузки.

### Что изменилось
Теперь каждый раздел загружается с сервера только один раз за визит — при
первом открытии. Дальше при переключении между вкладками переход
происходит мгновенно, без крутящегося колеса, потому что раздел просто
скрывается/показывается, а не создаётся заново. Разделы, которые вы ни
разу не открывали за визит, по-прежнему не загружаются заранее — экономит
трафик и нагрузку на сервер.
...
### Проверка
- Откройте «Базу знаний» (или любой раздел, кроме «Доски») — должен
  ненадолго появиться индикатор загрузки.
- Переключитесь на «Доску», затем обратно на «Базу знаний» — переход
  должен произойти мгновенно, без индикатора загрузки.
...
# Миграции БД (см. раздел 7 про схему)
...
# Python-зависимости backend (см. раздел 7 про gmpy2/libgmp-dev)
cd /var/www/era
...
## 7. Что дополнительно установить на VPS

### 7.1. Системная библиотека для быстрого шифрования (рекомендуется)
...
### 7.2. Обновите схему при ручном применении миграции
...
- Ни в одном файле этого обновления не используется `poehali.dev`/
  `cdn.poehali.dev` — запасной адрес по умолчанию (используется, только
  если `S3_ENDPOINT`/`S3_PUBLIC_URL` не заданы в `.env`) во всех трёх
  файлах (`backend/patches/index.py`, `backend/admin/index.py`,
  `backend/tasks/index.py`) указывает на локальный MinIO
  (`http://127.0.0.1:9000`), как и в остальных файлах проекта. Убедитесь,
  что в `.env` заданы `S3_ENDPOINT`/`S3_PUBLIC_URL` под ваш сервер (см.
  шаг 4 `deploy/README.md`).

...
CREATE TABLE IF NOT EXISTS ai_files (
    ...
    -- image/video/document — результат генерации. В лимит объёма (см. V0083) входят ВСЕ типы:
    -- место в хранилище одинаково занимают и загрузки сотрудника, и то, что создала модель.
...
-- Колонка заводится для совместимости, но БОЛЬШЕ НЕ ИСПОЛЬЗУЕТСЯ: лимит по количеству файлов
-- отменён, действует только лимит суммарного ОБЪЁМА (users.ai_size_limit_mb, см. V0083) —
-- количество плохо отражает нагрузку на хранилище (десяток видео весит больше сотен документов).
ALTER TABLE users ADD COLUMN IF NOT EXISTS ai_file_limit INTEGER NOT NULL DEFAULT 50;

COMMENT ON COLUMN users.ai_file_limit IS 'НЕ ИСПОЛЬЗУЕТСЯ: лимит по количеству файлов отменён, действует только лимит объёма (ai_size_limit_mb).';

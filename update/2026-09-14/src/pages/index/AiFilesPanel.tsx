...
export default function AiFilesPanel({ state, onClose }: { state: AiFilesState; onClose: () => void }) {
  const { files, totalSize, usedFiles, usedMb, limitMb, loading, busyId, clearing } = state;
  ...
  // Единственный лимит — объём. Считается по ВСЕМ файлам, включая сгенерированные.
  const nearLimit = limitMb > 0 && usedMb >= limitMb;
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="p-3 border-b border-border space-y-2 shrink-0">
        ...
        {/* Расход личного лимита объёма. Считаются ВСЕ файлы сотрудника — и загруженные им, и
            сгенерированные моделью: место в хранилище они занимают одинаково. */}
        <div className="space-y-1">
          ...
          <div className="flex items-center justify-between text-[11px] text-muted-foreground">
            <span>Занято: {usedMb} из {limitMb} МБ</span>
            <span>{usedFiles} файлов · {fmtFileSize(totalSize)}</span>
          </div>
        </div>
        ...
        {nearLimit && (
          <div className="text-[11px] text-destructive flex items-start gap-1.5">
            ...
            Место закончилось — очистите ненужные файлы, иначе новые загрузки и генерация
            будут недоступны
          </div>
        )}

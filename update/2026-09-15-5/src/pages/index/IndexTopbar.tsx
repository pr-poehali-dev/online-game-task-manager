...
export default function IndexTopbar({
  ...
  return (
    <>
      ...
      {/* Filter bar — search + server + sprint */}
      {view === 'board' && (
        <div className="flex items-center gap-2 px-3 sm:px-6 py-2.5 border-b border-border bg-card/10 overflow-x-auto scrollbar-thin">
          {/* Поиск по номеру задачи (#141 или просто 141 — точное совпадение) и по названию —
              см. taskMatchesSearch в sharedHelpers.ts. shrink-0 + фиксированная ширина, чтобы
              поле не терялось при горизонтальной прокрутке остальных фильтров. */}
          <div className="relative shrink-0 w-40 sm:w-56">
            ...
            <input
              ...
              className="w-full h-7 pl-8 pr-7 rounded-md border border-border bg-secondary/40 text-xs focus:outline-none focus:ring-1 focus:ring-primary"
            />
            {searchQuery && (
              <button

...
export const ERROR_MESSAGES: Record<string, string> = {
  ...
  size_limit_exceeded: 'Не хватает места в хранилище — очистите ненужные файлы в разделе «Мои файлы» или попросите администратора увеличить лимит',
  ...
  // AI Tunnel понимает 'auto' (автоподбор модели) только в текстовом чате — для изображений и
  // видео нужно выбрать конкретную модель вручную (см. backend/ai/generate.py).
  auto_not_supported: 'Автоподбор модели здесь не работает — выберите конкретную модель в списке',
  // invalid_param — backend сам отклонил параметр (соотношение сторон/фон/длительность и т.п.),
  // не поддерживаемый выбранной моделью, сверив запрос с каталогом ДО платного обращения к AI
  // Tunnel (см. backend/ai/common.py, _validate_media_params). message от backend уже содержит
  // конкретные допустимые значения — используется напрямую в errorText ниже.
  invalid_param: 'Этот параметр не подходит для выбранной модели — уточните значение и попробуйте снова',
};
...
export function errorText(err: string, message?: string, status?: number): string {
  ...
  // invalid_param — наш собственный backend, message уже готовый человекочитаемый текст с
  // точным списком допустимых значений для ВЫБРАННОЙ модели (см. _validate_media_params) —
  // конкретнее общей фразы из ERROR_MESSAGES.
  if (err === 'invalid_param' && message) return message;

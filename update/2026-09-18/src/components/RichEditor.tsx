...
// common — набор самых ходовых языков (js/ts/python/json/css/html/bash и т.п.), этого достаточно
// для внутреннего задачника и не тянет полный словарь lowlight (все языки — это лишний вес бандла).
...
export default function RichEditor({ content, onChange, placeholder, onImageUpload, large, toolbarExtra }: Props) {
  ...
  const editor = useEditor({
    extensions: [
      // codeBlock отключён у StarterKit — вместо него ниже подключён CodeBlockLowlight с
      // подсветкой синтаксиса, два расширения с одним именем узла конфликтовали бы.
      ...
      TableHeader,
      TableCell,
      // Выравнивание применяется только к текстовым блокам — на списках/таблице кнопки
      // просто не будут активны, это нормальное поведение TextAlign.
  ...
  return (
    <div className="rounded-xl border border-border bg-secondary/20 overflow-hidden">
      ...
      <div className="flex flex-wrap items-center gap-0.5 px-2 py-1.5 border-b border-border bg-card/60">
        ...
        {/* Блок кода — узел codeBlock из CodeBlockLowlight (заменяет собой codeBlock StarterKit,
            см. useEditor выше), с автоматической подсветкой синтаксиса по языку. Активен и по
            горячей клавише ``` . В отличие от инлайн-кода выше — своя строка с тёмным фоном,
            для логов/JSON и прочих крупных фрагментов. */}
        <ToolBtn title="Блок кода" active={editor.isActive('codeBlock')} onClick={() => editor.chain().focus().toggleCodeBlock().run()}>
          <Icon name="SquareCode" size={14} />
        </ToolBtn>

        <Divider />

        <ToolBtn title="По левому краю" active={editor.isActive({ textAlign: 'left' })} onClick={() => editor.chain().focus().setTextAlign('left').run()}>
          <Icon name="AlignLeft" size={14} />
        </ToolBtn>
        <ToolBtn title="По центру" active={editor.isActive({ textAlign: 'center' })} onClick={() => editor.chain().focus().setTextAlign('center').run()}>
          <Icon name="AlignCenter" size={14} />
        </ToolBtn>
        <ToolBtn title="По правому краю" active={editor.isActive({ textAlign: 'right' })} onClick={() => editor.chain().focus().setTextAlign('right').run()}>

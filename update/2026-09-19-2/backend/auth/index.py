...
def _effective_perms(role, raw):
    '''patch_edit/logs_view/ai_access/team_manage по умолчанию False даже для role == 'admin' (см.
    backend/admin/index.py, ТА ЖЕ функция продублирована там для action=list/set_permissions —
    держать оба списка ALL_PERMISSIONS и обе копии этой функции синхронными, иначе новое
    привилегированное право будет сохраняться в БД, но не возвращаться сотруднику при обновлении
    страницы: ai_access/logs_view раньше отсутствовали ЗДЕСЬ, из-за чего право, выданное через
    админку, никогда не долетало до самого сотрудника — ни при обновлении страницы, ни при
    перезаходе, потому что backend/auth/index.py (action=me/login) просто не включал их в ответ.
    patch_edit изначально есть только у OWNER_USER_ID через миграцию, logs_view/ai_access —
    чувствительные/платные разделы, team_manage делегируется точечно каждому участнику.
    patch_launcher_upload/patch_delete_files требуют patch_edit=true как предусловие (см.
    backend/admin/index.py за подробностями).'''
    result = {}

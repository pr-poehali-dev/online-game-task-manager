...
def _effective_perms(role, raw):
    ...
    backend/admin/index.py, ТА ЖЕ функция продублирована там и в backend/auth/index.py — держать
    списки ALL_PERMISSIONS и логику синхронными, иначе право, выданное через админку, сохраняется
    в БД, но не долетает до пользователя при входе через этот флоу: ai_access/logs_view раньше
    отсутствовали ЗДЕСЬ). patch_edit изначально есть только у OWNER_USER_ID через миграцию,
    logs_view/ai_access — чувствительные/платные разделы, team_manage делегируется точечно
    каждому участнику. patch_launcher_upload/patch_delete_files требуют patch_edit=true как
    предусловие (см. backend/admin/index.py за подробностями).'''
    result = {}

...
def _model_info(group, model_id):
    '''Достаёт описание ОДНОЙ модели из закешированного каталога (см. _fetch_models) — не делает
    отдельный сетевой запрос. Возвращает None, если каталог недоступен или модели в нём нет
    (тогда валидацию параметров лучше пропустить и отдать решение самому AI Tunnel, а не
    блокировать запрос из-за временного сбоя ЭТОГО похода за каталогом).'''
    try:
        return (_fetch_models(group) or {}).get(model_id)
    except Exception:
        return None


def _validate_media_params(group, model_id, params):
    '''Заранее (ДО платного запроса к /images/generations или /videos) отклоняет параметры,
    которые AI Tunnel строго проверяет по возможностям КОНКРЕТНОЙ модели и в ответ на
    несовпадение вернёт 400 (см. docs/ai-tunnel-api-reference.md, разделы "Генерация
    изображений"/"Генерация видео"): aspect_ratio/resolution/background должны входить в
    supported_* модели, n не может превышать max_n, generate_audio можно передавать только
    модели с generate_audio: true, frame_images — только с непустым supported_frame_images,
    duration видео — только из supported_durations. Остальные параметры (size, quality,
    output_format, seed) AI Tunnel сам молча игнорирует у моделей без поддержки — их не
    проверяем, блокировать нечего.

    Возвращает None, если всё в порядке (или каталог модели сейчас недоступен — тогда решение
    остаётся за AI Tunnel), иначе (error_code, understandable_message) для _bad_message.'''
    info = _model_info(group, model_id)
    if not info:
        return None

    def _bad_param(field, value, supported):
        allowed = ', '.join(str(x) for x in supported) if supported else '—'
        return ('invalid_param', f'Модель «{model_id}» не поддерживает {field}={value}. Доступные значения: {allowed}.')

    aspect_ratio = params.get('aspect_ratio')
    supported_ar = info.get('supported_aspect_ratios')
    if aspect_ratio and supported_ar is not None and aspect_ratio not in supported_ar:
        return _bad_param('соотношение сторон', aspect_ratio, supported_ar)

    background = params.get('background')
    supported_bg = info.get('supported_background')
    if background and supported_bg is not None and background not in supported_bg:
        return _bad_param('фон', background, supported_bg)

    n = params.get('n')
    max_n = info.get('max_n')
    if n and max_n is not None and int(n) > max_n:
        return ('invalid_param', f'Модель «{model_id}» отдаёт максимум {max_n} изображени{"е" if max_n == 1 else "й"} за раз, запрошено {n}.')

    if group == 'videos':
        resolution = params.get('resolution')
        supported_res = info.get('supported_resolutions')
        if resolution and supported_res is not None and resolution not in supported_res:
            return _bad_param('разрешение', resolution, supported_res)

        duration = params.get('duration')
        supported_dur = info.get('supported_durations')
        if duration and supported_dur is not None and int(duration) not in supported_dur:
            return _bad_param('длительность', duration, supported_dur)

        if params.get('generate_audio') is False and info.get('generate_audio') is not True:
            return ('invalid_param', f'Модель «{model_id}» не умеет переключать звук — уберите параметр звука.')

        if params.get('frame_images') and not info.get('supported_frame_images'):
            return ('invalid_param', f'Модель «{model_id}» не поддерживает опорные кадры (image-to-video) — уберите кадры или выберите другую модель.')
...
# Модель ОБЫЧНАЯ, не «рассуждающая»: рассуждающие тратят весь бюджет токенов на размышления и
# возвращают пустую строку — название диалога не появлялось вовсе.
...
# Кеширование промпта (см. docs/ai-tunnel-api-reference.md, "Кеширование промпта"): повторяющийся
# ПРЕФИКС запроса можно не тарифицировать по полной цене — часть провайдеров (OpenAI/DeepSeek/
# Gemini/Grok/Kimi/GLM) кеширует стабильное начало сообщений сама, без настроек, но Claude и Qwen
# требуют явную точку `cache_control: {"type": "ephemeral"}` на текстовом блоке — без неё кеша не
# будет вовсе. Формат ниже — Anthropic-style, AI Tunnel сам конвертирует его в нужный провайдеру
# синтаксис (на GPT-5.6+ — в prompt_cache_breakpoint, на прочих — молча игнорирует, если провайдеру
# это не нужно, это НЕ ошибка). CODE_SYSTEM_PROMPT — самый частый повторяющийся префикс во всём
# разделе "AI": ОДИН И ТОТ ЖЕ текст на каждое сообщение режима «код» у КАЖДОГО сотрудника —
# идеальный кандидат на кеш, экономия растёт с числом сообщений в диалоге (весь системный промпт
# входит в кэшируемый префикс каждого следующего хода).
def _cacheable_text(text: str) -> list:
    return [{'type': 'text', 'text': text, 'cache_control': {'type': 'ephemeral'}}]


def _cacheable_system_message(text: str) -> dict:
    '''Системное сообщение с явной точкой кеша — см. _cacheable_text выше. Используется вместо
    обычного {'role': 'system', 'content': text} там, где системный промпт неизменен между
    запросами (сейчас — только CODE_SYSTEM_PROMPT в режиме «код»).'''
    return {'role': 'system', 'content': _cacheable_text(text)}
...
def _chat_to_dict(row):
    cid, title, mode, model, pinned, created_at, updated_at, web_search_enabled = row
    return {
...
def _message_to_dict(row):
    '''Строка ai_messages → сообщение для фронта. Последними элементами row могут идти признак
    наличия doc_spec (собранный офисный документ) и reasoning (ход рассуждений модели, см.
    db_migrations V0103) — по doc_spec интерфейс показывает у сообщения кнопку «Доработать». Сам
    doc_spec наружу не отдаём: он нужен только серверу при правке, а объём у него приличный (вся
    структура таблицы).'''

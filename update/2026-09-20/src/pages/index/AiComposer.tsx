...
interface AiComposerProps {
  ...
  // webSearch — состояние переключателя "Использовать веб-поиск" (см. useAiSection.ts), имеет
  // смысл только для chat/code — показывается только там (см. AiChatPane.tsx).
  webSearch?: boolean;
  onWebSearchChange?: (enabled: boolean) => void;
  // webSearchSupported — умеет ли ВЫБРАННАЯ модель пользоваться инструментом (см. AiTypes.ts,
  // web_search_cost). Если нет — переключатель показываем отключённым с подсказкой, а не прячем
  // совсем, чтобы сотрудник понимал, почему поиск не сработает, а не терялся в догадках.
...
export default function AiComposer({
  ...
  webSearch = false, onWebSearchChange, webSearchSupported = true,
  ...
  return (
    <div className="border-t border-border p-3 sm:p-4 shrink-0">
      ...
      {/* Переключатель "Использовать веб-поиск" — только chat/code (см. AiChatPane.tsx, document
          использует свою модель, image/video — отдельный композер AiGenerateComposer). Дизаблится,
          если у ВЫБРАННОЙ модели нет web_search_cost в каталоге (см. AiTypes.ts) — модель просто
          не умеет пользоваться инструментом, включённый чекбокс ни на что не повлиял бы. */}
      {(mode === 'chat' || mode === 'code') && onWebSearchChange && (
        <button
          type="button"
          onClick={() => onWebSearchChange(!webSearch)}
          disabled={!webSearchSupported}
          title={webSearchSupported ? 'Модель сможет искать актуальную информацию в интернете' : 'Выбранная модель не умеет искать в интернете'}
          className={`mb-2 flex items-center gap-1.5 h-7 px-2.5 rounded-lg border text-xs font-medium transition-colors disabled:opacity-40 disabled:cursor-not-allowed ${
        ...
        >
          ...
          Веб-поиск
          {webSearch && webSearchSupported && <Icon name="Check" size={12} />}
        </button>
      )}

...
export interface AiChatSummary {
  ...
  // webSearchEnabled — состояние чекбокса "Использовать веб-поиск" НАД полем ввода (см.
  // AiComposer.tsx), сохраняется per-chat в ai_chats.web_search_enabled вместе с моделью — не
  // сбрасывается при переключении между диалогами (см. backend/ai/generate.py).
...
export interface AiMessage {
  ...
  hasDocSpec?: boolean;
  // reasoning — цепочка мыслей "думающей" модели ДО итогового ответа (см. backend/ai/common.py,
  // docs/ai-tunnel-api-reference.md, "Токены рассуждений"). Есть не у всех моделей — часть моделей
  // (серия OpenAI o) думает, но текст мыслей не отдаёт вовсе. Показывается сворачиваемым блоком
  // «Ход рассуждений» над ответом (см. AiMessageList.tsx, ReasoningPanel).
...
export interface AiModelInfo {
  ...
  // backend/ai/generate.py — инструмент подключается, когда сотрудник включил чекбокс "Веб-поиск"
  // над полем ввода, см. AiComposer.tsx). Поля нет = модель поиском не пользуется вовсе.
